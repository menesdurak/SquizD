...
class GetAllWordsFromCategoryUseCase @Inject constructor(
    private val localRepository: LocalRepository
) {

    suspend operator fun invoke(categoryId: Int): Resource<List<Word>> {
        return try {
            Resource.Success(localRepository.getAllWordsFromCategory(categoryId))
        } catch (e: IOException) {
            Resource.Error(e)
        }
    }
